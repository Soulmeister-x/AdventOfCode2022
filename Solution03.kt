fun main() {
    var sum = 0
    var sumOfGroups = 0
    var groupSet = setOf<Char>()
    val bags = input.split("\n")
    bags.forEachIndexed { index, bag ->
        val letter = findDuplicate(bag)
        val prio = getCode(letter)
        val setB = bag.toSet()

        if (index%3 == 0)
            groupSet=setB
        groupSet = groupSet.intersect(setB)
        println("$index# groupset: $groupSet")

        if (((index+1) % 3) == 0) {
            sumOfGroups += getCode(groupSet.first())
            println("sum: $sumOfGroups | groupset: $groupSet")
        }
        sum += prio
    }
    println("$sum = sum")
...
private fun findDuplicate(bag: String): Char {
    ...
    var first = sub1.toSet()
    var second = sub2.toSet()
    ...
    first.forEach { if (second.contains(it)) return it }
    return ' '
}
